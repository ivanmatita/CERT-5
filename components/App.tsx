...
const App = () => {
  ...
  const handleSaveInvoice = async (inv: Invoice, sId: string, action?: string) => {
      ...
      if (action === 'CERTIFY') {
          // 1. Sequencialidade: Não pode certificar o Doc 2 se o Doc 1 (mesmo tipo/série) for rascunho
          ...
          // 2. Cronologia: Data não pode ser anterior ao último certificado
      ...
      // REGRA AGT: Atribuição de número fiscal definitivo apenas na certificação
      if (action === 'CERTIFY' && !inv.isCertified) {
          ...
          // --- BAIXA DE STOCK AUTOMÁTICA NA CLOUD ---
          // Apenas para documentos que representam entrega de bens
          if ([InvoiceType.FT, InvoiceType.FR, InvoiceType.VD, InvoiceType.GE].includes(finalInv.type)) {
                  ...
                  if (item.productId && item.type === 'PRODUCT') {
                      try {
                          await supabase.from('movimentos_stock').insert({
                              ...
                              notas: `Baixa automática via Certificação AGT: ${finalInv.number}`,
          ...
          // Rascunhos não têm número fiscal
          finalInv.number = '---';
      }
  ...
  const handleDeleteInvoice = async (id: string) => {
      ...
      if (inv?.isCertified) {
          alert("REGRA AGT: Documentos certificados não podem ser apagados do sistema. Utilize a função de anulação para manter a rastreabilidade fiscal.");
          return;
      }
  ...
  const handleCancelInvoice = async (id: string, reason: string) => {
      ...
      try {
          ...
          console.error("Erro anulação:", err); 
      } finally { 
          setIsLoading(false); 
      }
  };
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    try {
        ...
        console.error("Erro delete compra:", err);
    }
  };
